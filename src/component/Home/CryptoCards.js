    ...
    <div>
      <div className="hero">
        ...
        <div className="searchInput">
          <input className="inputSearchBox" placeholder="Search coin"
            type="text" value={searchcoin} onChange={onSearch}/>
        </div>
      </div>
      
      <div className="cardsContain">
        {status === 'pending' ?
         ( <div className="ring">Loading<span /></div> ) 
         : 
         ( filteredSearch.map((coin) => (
            <div key={coin.id} className="conisCard">

              <div className="coinIcon">
                <img className="coinIconImg"
                 src={coin.icon} alt="CoinIcon" />
              </div>
              ...
              <div className="coinDetails">
                ...
                <p className="price">${coin.price.toFixed(3)}</p>
              </div>

              <Link className="enterIcon" to={`${coin.id}`}>
                <div className="arrowForward"><p>➡</p></div>
              </Link>
            </div>)) )}
      </div>
    </div>
